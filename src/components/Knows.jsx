...
export default function Knows({ values }) {
  ...
  return (
    <Row gutter={[16, 16]}>
      {metrics.map(({ label, value, detail, delta, color }, i) => {
        const card = (
          <Card size="small">
        ...
        // Wrap busiest daypart in a black ribbon with white Fire icon + text
        if (label === busiestLabel) {
          return (
            <Col key={i} xs={24} sm={12} md={8} lg={6}>
              <Badge.Ribbon
                text={
                  <span style={{ color: '#fff' }}>
                    <FireTwoTone twoToneColor="red" style={{ marginRight: 4 }} />
                    Busiest Time
                  </span>
                }
                color="#000"
